 ...
 *   isString({}) => false
 *   isString('test') => true
 *   isString(new String('test')) => true (instanceof only for objects)
 *
 */
...
function sumOfCodes(str) {
  return str?.split('').reduce((acc, corr) => acc + corr.charCodeAt(), 0) || 0;
}
...
function isPalindrome(str) {
  const noSigns = str.toLowerCase().replace(/[^a-z0-9]/gi, '');
  return noSigns === noSigns.split('').reverse().join('');
}
  ...
  return sentence
    ...
    .reduce(
      (longestSubStr, subString) =>
        subString.length > longestSubStr.length ? subString : longestSubStr,
      ''
    );
}
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((e) => {
      const index = normal.indexOf(e);
      return index === -1 ? e : rot[index];
    })
    .join('');
}
